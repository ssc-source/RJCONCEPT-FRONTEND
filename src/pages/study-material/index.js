            ...
            <article key={material.id} className="group rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              <div className="relative h-56 overflow-hidden rounded-[1.5rem] bg-slate-100">
                {material.coverImageUrl ? (
                  <img src={resolveMediaUrl(material.coverImageUrl)} onError={handleMediaError} alt={material.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                ) : material.fileKind === 'IMAGE' ? (
                  <img src={resolveMediaUrl(`/api/study-materials/public/${material.id}/view`)} onError={handleMediaError} alt={material.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                ) : (
